...
#if defined(NDEBUG)
# error "ReviveCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe945301274646100a17b1788bf7aa0b718f72c4e75b5b6c29b03f4fe1ad62ad3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ReviveCoin: starting difficulty is 1 / 2^12
...
{
    // ReviveCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ReviveCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ReviveCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1503687912, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "25 Aug 2017 Samsung heir Lee Jae yong jailed for corruption";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 42 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fcef1c633b8420aea53aff64d9be722baba75e28dcc45cffe2920c0cd5871d593f83c6b2d42d18be2591ebd9ce79da6a2602966981e4d851a8d47406e92a3edb") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1503687912;
        ...
        assert(block.hashMerkleRoot == uint256("0x8036c118dd96986205d27cf218455a14834266906708d6ade278ca22234d82e6"));
...
unsigned char pchMessageStart[4] = { 0xe8, 0x86, 0xae, 0x66 }; // ReviveCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ReviveCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ReviveCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ReviveCoinMiner(CWallet *pwallet)
{
    printf("ReviveCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running ReviveCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ReviveCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ReviveCoinMiner, pwallet));
}
